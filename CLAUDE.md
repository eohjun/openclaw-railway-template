...
7. **Control UI requires allowInsecureAuth to bypass pairing** → `gateway.controlUi.allowInsecureAuth=true` is set during onboarding AND re-applied on every gateway start (doctor migrations can reset config). Prevents "disconnected (1008): pairing required" errors (GitHub issue #2284). Wrapper already handles bearer token auth, so device pairing is unnecessary.
